...
export async function POST(req: Request) {
  ...
  if (typeof boardContext === "string" && boardContext.trim()) {
    // boardContext comes from the in-panel Agine Chat (game review /
    // position pages), not free-form user text — it's generated by
    // buildAnalysisChatContext from state already on the page (FEN,
    // PGN, engine lines, game review). Available on every tier since
    // it's the core of that feature, not a premium add-on. Truncated
    // defensively even though the client already caps it.
    const trimmedBoardContext =
      new TextEncoder().encode(boardContext).length > MAX_BOARD_CONTEXT_BYTES
        ? boardContext.slice(0, MAX_BOARD_CONTEXT_BYTES) + "\n…(truncated)"
        : boardContext;

    systemParts.push(
      "## Live Board Context",
      "The user is looking at this exact chess position/game right now in the analysis panel. " +
        "Use it directly to answer — you generally don't need to call chess tools for information already given here.",
      "",
      trimmedBoardContext
    );
  }
